...
//        telemetry.addData("filteredRects.size()", filteredRects.size());
        ...
        for (int i = 0; i < filteredRects.size(); i++) {
            RotatedRect rect = filteredRects.get(i);
            ...
            // Store the angle along with the position
            double[] positionWithAngle = { tempRealPositions.get(i)[0], tempRealPositions.get(i)[1], Math.toRadians(procAngle) };
            tempRealPositions.set(i, positionWithAngle);
        }
