...
public class ServoAngleControl extends LinearOpMode {
    ...
    private RobotConfig robotConfig; // Definirea configurației robotului

    @Override
    public void runOpMode() {
        // Inițializarea hardware-ului
        double servoPosition = robotConfig.outtakeClawServo.getPosition();
        // Așteptăm să înceapă
        ...
        while (opModeIsActive()) {
            ...
            servoPosition += stickY * 0.005; // Incrementăm/decrementăm poziția servo-ului

            servoPosition = Math.max(0.0, Math.min(1.0, servoPosition)); // Clamping între 0.0 și 1.0

            // Setăm poziția servo-ului
            robotConfig.outtakeClawServo.setPosition(servoPosition);
