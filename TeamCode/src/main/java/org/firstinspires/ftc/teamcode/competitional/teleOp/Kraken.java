...
//            }
            if(robotConfig.upMotor.getCurrentPosition()>10)
                areSlidesDown = false;
            if(robotConfig.upMotor.getCurrentPosition()<60 && robotConfig.upMotor.getVelocity()<0.05 && targetPosition== OuttakeConstants.OUTTAKE_MIN_POSITION){
...
//                setOuttakeState(OuttakeState.INIT);
            ...
            case WAITING:
                ...
                if((transientState.milliseconds()>300 && !fastTransfer) || (transientState.milliseconds()>400 && fastTransfer)){
                    TransferTimer.reset();
                    setOuttakeState(OuttakeState.PICKUP);
