...
//@Config
@Autonomous(name = "🐙 SampleAuto (6+0) 🐙")
public class SpecAutoWithMove extends OpMode {
    ...
    public double retract = 0.06;// Smaller step size to ensure smoother movement (smaller change)
    ...
    private int sub2X = 68;

    private final Pose sub1 = new Pose(sub1X, 93, Math.toRadians(-90));

    private final Pose subLeft = new Pose(64.2, 93, Math.toRadians(-90));

    private final Pose subRight = new Pose(61.8, 93, Math.toRadians(-90));


    private final Pose sub2 = new Pose(sub2X, 92, Math.toRadians(-90));

    private final Pose sub2Left = new Pose(69,92,Math.toRadians(-90));

    private final Pose sub2Right = new Pose(67,92,Math.toRadians(-90));

    private final Pose scoreFinal = new Pose(19,133,Math.toRadians(315));

    private final Pose subControl = new Pose(65, 118, Math.toRadians(-90));

    private final Pose subControlFinal = new Pose(65, 125, Math.toRadians(-90));

    private final Pose parkFinal = new Pose(65, 95, Math.toRadians(-90));



    private PathChain park, subPath2, swipeLeft2,swipeRight2,return2,swipeLeft, swipeRight,return1, scorePreload, firstPickup, secondPickup, thirdPickup, subPath, score1, score2, score3; //?
    ...
    public void buildPaths() {
        ...
        return1 = follower.pathBuilder()
                .addPath(new BezierCurve(new Point(sub1), new Point(subControl), new Point(scoreFinal)))
        ...
        return2 = follower.pathBuilder()
                .addPath(new BezierCurve(new Point(sub2), new Point(subControl), new Point(scoreFinal)))
        ...
        park = follower.pathBuilder()
                .addPath(new BezierCurve(new Point(scoreFinal), new Point(subControlFinal), new Point(parkFinal)))
                .setLinearHeadingInterpolation(scoreFinal.getHeading(),parkFinal.getHeading())
                .addParametricCallback(0, ()-> follower.setMaxPower(0.7))
                .addParametricCallback(0.2, ()-> setSliderMinPosition())
                .build();
            ...
            case 9:
                if(!follower.isBusy() && !sampleFound) {
                    ...
                    if (cameraTimer.seconds() > 0.5 && !sampleFound ) {
                        ArrayList<double[]> samples2 = processor.getRealPositions();
                        ArrayList<Double> sampleAngles = processor.getSampleAngles();
                        for (int i = 0; i < samples2.size(); i++) {
                                sampleX = samples2.get(i)[0];  // X position of the sample
                                sampleY = samples2.get(i)[1];
                            if (sampleX > X_MIN && sampleX < X_MAX && sampleY <=0.5  && sampleY >=-1.5) {
                                sampleFound = true;
                                sampleAngle = sampleAngles.get(i);
                                sampleId = i;
                                break;
                                //   setPathState(12);
                            }
                            if (sampleX <= X_MIN && sampleY <= 0.5 && sampleX>3  && sampleY >=-1.5) {
                                sampleId = i;
                                sampleAngle = sampleAngles.get(i);
                                sampleFound = true;
                            ...
                            if (sampleX >= X_MAX && sampleY <= 0.5 && sampleX<3  && sampleY >=-1.5) {
                                sampleAngle = sampleAngles.get(i);
                                sampleFound = true;
                                sampleId = i;
                ...
                if(sampleFound && !follower.isBusy()){
                    if(!canpickUp) {
                        ArrayList<double[]> samples2 = processor.getRealPositions();
                        ArrayList<Double> sampleAngles2 = processor.getSampleAngles();
//
                        double minDistance = Double.MAX_VALUE;
                        int closestSampleIndex = -1;

                        for (int i = 0; i < samples2.size(); i++) {
                            double x = samples2.get(i)[0];
                            double y = samples2.get(i)[1];

                            double distance = Math.hypot(sampleX - x, sampleY - y);
                            double yDifference = Math.abs(sampleY - y);

                            if (yDifference < y_Threshold) {
                                minDistance = distance;
                                closestSampleIndex = i;
                                sampleAngle2 = sampleAngles2.get(closestSampleIndex);
                            }
                        }
                        //robotConfig.intakeWristRotServo.setPosition(sampleAngle);
                        robotConfig.intakeWristRotServo.setPosition(mapAngleToServoPosition(sampleAngle2));
                        robotConfig.intakeWristRotServo.setPosition(mapAngleToServoPosition(sampleAngle2));
                    ...
                    if(pickupTimer.seconds()>0.4 && canpickUp && !canDown){
                    ...
                    if(pickupTimer.seconds()>0.6 && canpickUp){
            ...
            case 16:
                ...
                if(hasScored && retractTimer.seconds()>0.5) {
            ...
            case 18:
                if(!follower.isBusy() && !sampleFound) {
                    ...
                    robotConfig.intakeWristLeftServo.setPosition(ServoConstants.INTAKE_WRIST_LEFT_CAMERA_POSITION);
                    if (cameraTimer.seconds() > 0.5 && !sampleFound ) {
                        ArrayList<double[]> samples3 = processor.getRealPositions();
                        ArrayList<Double> sampleAngles3 = processor.getSampleAngles();
                        for (int i = 0; i < samples3.size(); i++) {
                            sampleX = samples3.get(i)[0];  // X position of the sample
                            sampleY = samples3.get(i)[1];
                            if (sampleX > X_MIN && sampleX < X_MAX && sampleY <=0.5  && sampleY >=-1.5) {
                                sampleFound = true;
                                canpickUp = false;
                                sampleAngle = sampleAngles3.get(i);
                                sampleId = i;
                                break;
                                //   setPathState(12);
                            }
...
//                            if (sampleX > X_MIN && sampleX < X_MAX && sampleY <=0.5  && sampleY >=-1.5) {
//                                sampleFound = true;
//                                sampleAngle3 = sampleAngles3.get(i);
//                                sampleId = i;
//                                break;
//                                //   setPathState(12);
//                            }
...
//                        ));
                        ...
                        for (int i = 0; i < samples3.size(); i++) {
                            double x = samples3.get(i)[0];
                            double y = samples3.get(i)[1];

                            double yDifference = Math.abs(sampleY - y);

                            if (yDifference<y_Threshold) {
                                closestSampleIndex = i;
                                sampleAngle3 = sampleAngles3.get(closestSampleIndex);
                            }
                        }
                        robotConfig.intakeWristRotServo.setPosition(mapAngleToServoPosition(sampleAngle3));
                        currentPosition = robotConfig.intakeElbowLeftServo.getPosition();
                        robotConfig.outtakeClawServo.setPosition(ServoConstants.OUTTAKE_CLAW_OPEN_POSITION);
                        canpickUp = true;
                        canDown = false;
    ...
    public void loop() {
        ...
        controlPID();
        telemetry.addData("intakeElbow", robotConfig.intakeElbowLeftServo.getPosition());
