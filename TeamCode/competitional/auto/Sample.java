...
public class Sample extends OpMode {
    ...
    private static final double EXTEND_STEP_SIZE2 = 0.01;
    private static final double X_MIN = -2.5;
    private static final double X_MAX = 0.6;
    public double retract = 0.08;// Smaller step size to ensure smoother movement (smaller change)
    ...
    private final Pose subLeft = new Pose(65, 96, Math.toRadians(-90));

    private final Pose subRight = new Pose(63, 96, Math.toRadians(-90));


    private final Pose sub2 = new Pose(sub2X, 91, Math.toRadians(-90));

    private final Pose sub2Left = new Pose(69,96,Math.toRadians(-90));

    private final Pose sub2Right = new Pose(67,96,Math.toRadians(-90));
...
//                            }
            ...
            case 10:
                if(!follower.isBusy()) {
                    ...
                    if(pickupTimer.seconds()>0.7 && canpickUp){
                    ...
                    if(pickupTimer.seconds()>1.2 && canpickUp){
            ...
            case 14:
                if(!follower.isBusy()) {
                    ...
                    if(pickupTimer.seconds()>1 && canpickUp){
                        robotConfig.intakeClawServo.setPosition(ServoConstants.INTAKE_CLAW_CLOSED_POSITION);
                        startTransferTimer = true;
                        hasTransfered = false;
                        setPathState(15);
